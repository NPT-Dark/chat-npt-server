...
const {
  ...
  UpdateSeen,
  AddNotification,
} = require("./src/services/ioServices");
...
app.use(
  cors({
    origin: [
      ...
      "http://localhost:3001",
      "http://localhost:3002",
      "https://chat-npt-client.vercel.app",
      "chat-npt-client-khd43ytl0-npt-dark.vercel.app",
      "https://chat-npt-client-git-s3-npt-dark.vercel.app",
    ],
...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", async (data) => {
    ...
    socket.leave(data.id_User_Receive)
  });
  ...
  socket.on("update_status", async (data) => {
    ...
    if (listFriend.length !== 0) {
      ...
      for (const id_Friend of array) {
        socket
          .to(id_Friend)
          .emit("receive_friend_status", "Have a friend who is online");
        socket.leave(id_Friend)
      }
    }
  });
  //*Seen Message */
  socket.on("send_seen_message", async (data) => {
    ...
    if (update) {
      ...
      if (message.length > 0) {
        socket
          .to(data.id_User_Receive)
          .emit("receive_seen_message", message[message.length - 1].id_Message);
        socket.leave(data.id_User_Receive)
      }
    }
  });
  ...
  socket.on("send_invitation", async (data) => {
    if ((await ExistFriendRq(data)) === null) {
      ...
      socket
        .to(data.id_User_Recieve)
        .emit("receive_notification", "you received a notification !");
      socket.leave(data.id_User_Receive)
    }
  });
  //**Cancel Invitaiton */
  socket.on("send_cancel_invitation", async (data) => {
    ...
    socket
      .to(data.id_User_Recieve)
      .emit("receive_cancel_invitation", "Cancel successfully");
    socket.leave(data.id_User_Receive)
  });
  //**Accept Invitation */
  socket.on("accept_invitation", async (data) => {
    ...
    socket
      .to(data.id_User_Add)
      .emit(
        "receive_accept_invitation",
        `${
          userDetail.dataValues.firstName + " " + userDetail.dataValues.lastName
        } has accepted your friend request`
      );
    socket
      .to(data.id_User_Add)
      .emit("receive_notification", "you received a notification !");
    socket.leave(data.id_User_Add)
  });
  //** Unfriend */
  socket.on("send_unfriend", async (data) => {
    ...
    socket
      .to(data.id_User_Unfriend)
      .emit("receive_notification", "you received a notification !");
    socket.leave(data.id_User_Unfriend)
  });
  //** Disconnect*/
